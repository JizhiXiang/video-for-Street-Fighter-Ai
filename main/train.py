...
def make_env(game, state, seed=0):
    def _init():
        ...
        env = StreetFighterCustomWrapper(env,rendering=False)  #rendering为true表示能可视化
        env = Monitor(env)
        env.seed(seed)
        return env
    return _init
...
def main():
    ...
    checkpoint_interval = 1000 #31250 # checkpoint_interval * num_envs = total_steps_per_checkpoint
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        model.learn(
            total_timesteps=int(5000), #100000000 # total_timesteps = stage_interval * num_envs * num_stages (1120 rounds)
